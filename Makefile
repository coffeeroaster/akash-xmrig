...
# Remove dependency on ledger for now
create_wallet:
	$(AKASH_BIN) keys add "$(KEY_NAME)" ${KEYRING_OPT}
...
create_deployment:
	echo $(AKASH_BIN) tx deployment create $(DEPLOY_PATH)/deploy.yml --from $(KEY_NAME) --node $(AKASH_NODE) --chain-id $(AKASH_CHAIN_ID) --fees $(FEES) ${KEYRING_OPT}
	$(AKASH_BIN) tx deployment create $(DEPLOY_PATH)/deploy.yml --from $(KEY_NAME) --node $(AKASH_NODE) --chain-id $(AKASH_CHAIN_ID) --fees $(FEES) ${KEYRING_OPT}

update_deployment:
	echo $(AKASH_BIN) tx deployment update $(DEPLOY_PATH)/deploy.yml --from $(KEY_NAME) --node $(AKASH_NODE) --chain-id $(AKASH_CHAIN_ID) --fees $(FEES) ${KEYRING_OPT} --dseq ${DSEQ}
...
restart_lease:
	@test $${AKASH_ADDRESS?Please set environment variable AKASH_ADDRESS}
	$(AKASH_BIN) tx deployment group start list --owner $(AKASH_ADDRESS) --chain-id $(AKASH_CHAIN_ID) --node $(AKASH_NODE)
list_lease:
	@test $${AKASH_ADDRESS?Please set environment variable AKASH_ADDRESS}
...
list_lease_all:
	@test $${AKASH_ADDRESS?Please set environment variable AKASH_ADDRESS}
	$(AKASH_BIN) query market lease list --chain-id $(AKASH_CHAIN_ID) --node $(AKASH_NODE)

list_lease_status_all:
	@test $${AKASH_ADDRESS?Please set environment variable AKASH_ADDRESS}
	$(AKASH_BIN) query market lease list --chain-id $(AKASH_CHAIN_ID) --node $(AKASH_NODE)

list_deployment_all:
	@test $${AKASH_ADDRESS?Please set environment variable AKASH_ADDRESS}
	$(AKASH_BIN) query deployment list --chain-id $(AKASH_CHAIN_ID) --node $(AKASH_NODE)
...
lease_logs:
	$(AKASH_BIN) provider lease-logs -f --trace --node $(AKASH_NODE) --from $(KEY_NAME) --dseq $(DSEQ) --oseq $(OSEQ) --gseq $(GSEQ) --provider $(PROVIDER) ${KEYRING_OPT}

send_manifest:
	$(AKASH_BIN) provider send-manifest $(DEPLOY) --node $(AKASH_NODE) --dseq $(DSEQ)  --from $(KEY_NAME) --provider $(PROVIDER) $(KEYRING_OPT)
